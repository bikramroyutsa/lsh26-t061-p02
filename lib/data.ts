...
export function generateSampleData(): Medicine[] {
  ...
  // We want to generate exactly 42 items.
  // 10 Expired items (offsets < 0)
  // 10 Expiring <= 30 Days (offsets 0 to 30)
  // 10 Expiring 31-90 Days (offsets 31 to 90)
  // 12 Safe items (offsets > 90)
  ...
  for (let i = 0; i < 10; i++) {
    ...
    items.push({
      ...
      expiryDate: getTodayOffsetString(expiredOffsets[i]),
      returned: false,
    });
  }

  // 2. Expiring <= 30 Days (10 items)
  // Includes critical edge cases: 0 (today) and 30 days
  ...
  for (let i = 0; i < 10; i++) {
    ...
    items.push({
      ...
      expiryDate: getTodayOffsetString(soonOffsets[i]),
      returned: false,
    });
  }

  // 3. Expiring 31-90 Days (10 items)
  // Includes critical edge cases: 31 and 90 days
  ...
  for (let i = 0; i < 10; i++) {
    ...
    items.push({
      ...
      expiryDate: getTodayOffsetString(midOffsets[i]),
      returned: false,
    });
  }

  // 4. Safe Items (12 items)
  // Includes critical edge case: 91 days
  ...
  for (let i = 0; i < 12; i++) {
    ...
    items.push({
      ...
      expiryDate: getTodayOffsetString(safeOffsets[i]),
      returned: false,
    });
  }
