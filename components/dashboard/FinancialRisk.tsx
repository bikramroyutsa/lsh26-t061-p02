...
export default function FinancialRisk({ medicines }: FinancialRiskProps) {
  ...
  const renderRiskItem = (item: any, idx: number) => (
    <div
  ...
  return (
    <div className="clay-card p-6 md:p-8 flex flex-col h-full">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
        ...
        {/* ── Right: Top highest value at risk (Separated) ────────────── */}
        <div>
          <div className="flex items-center gap-2 mb-7">
          ...
          {highestRisks.length === 0 ? (
            ...
            <div className="space-y-4">
              {expiredRisks.length > 0 && (
                <div>
                  <h4 className="text-[10px] font-bold uppercase tracking-widest text-expired mb-2 pl-3">Already Expired</h4>
                  <div className="space-y-1">
                    {expiredRisks.map(renderRiskItem)}
                  </div>
                </div>
              )}
              ...
              {atRiskRisks.length > 0 && (
                <div>
                  <h4 className="text-[10px] font-bold uppercase tracking-widest text-warn mb-2 pl-3 mt-4">At Risk (≤30 Days)</h4>
                  <div className="space-y-1">
                    {atRiskRisks.map(renderRiskItem)}
                  </div>
                </div>
              )}
            </div>
